function [out,setupSWS]=FindSWS_TTP_Multiwave_Each(inputTOF,outplanes,setupdataprocessing,setupSWS,setupMakePlanes,dataDir)

% Estimate SWS at each rotation angle using the TTP method (all lateral range methods) WITH MULTIWAVE DETECTION
% inputTOF contains the TTP data at each rotation angle, lateral position, for the two highest local peaks
...
for iangle=1:length(outplanes)
    [out(iangle).SH,out(iangle).SecondSW]=fitSWStoTOF(outplanes(iangle),squeeze(inputTOF(iangle,:,:)),setupSWS.SH(iangle),setupSWS.outlierTOFthres,setupSWS.typeoffit);
end
...
if setupSWS.fignum
    ...
    for iangle=1:length(out)
        ...
        plot(out(iangle).SecondSW.times,out(iangle).SecondSW.latpos,'r')
        ...
        plot(out(iangle).SecondSW.latpos,out(iangle).SecondSW.times,'r-','LineWidth',1)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [outSH,outSecondWave]=fitSWStoTOF(outplane,TOF,setupSWSindividual,outlierTOFthres,typeoffit)
...
if all(isnan(TOFfit))
    outSH=NanOut(latpos,TOFfit,TOF);
    outSecondWave=outSH;
    return
end
...
switch typeoffit
    case 'setlat'
        % SH fit
        outSH=linearfit(TOFfit(:,1),latpos,setupSWSindividual,outlierTOFthres); % only use biggest peak for SH at first

        [outSH,outSecondWave]=fitSecondWave(latpos,TOFfit,TOF,outSH,setupSWSindividual,outlierTOFthres);

    case 'dynamiclat'
        outSH=dynamiclatlinearfit(TOFfit,latpos,setupSWSindividual,outlierTOFthres);

end

outSH.TOF=TOF;
outSH.hittingedgeflag=0; %for plotting
...
if size(TOFfit,2)==2
    TOFfit=[TOFfit(:,1);TOFfit(:,2)];
    latpos=[latpos, latpos];
end
...
function [out1,out2]=fitSecondWave(latpos,TOFfit,TOF,out1,setupSWSindividual,outlierTOFthres)
% try SV fit
TOFfittmp=TOFfit;TOFfittmp(out1.iTOFinlier)=NaN; % nan out anything in SH
if all(isnan(TOFfit))
    out2=NanOut(latpos,TOFfit,TOF);
    return; % give up if no points left
end

out2=linearfit(TOFfittmp,latpos,setupSWSindividual,outlierTOFthres); % use any remainders for SV
if or(out2.R2<0.7,length(out2.iTOFinlier)<4)
    out2=NanOut(latpos,TOFfit,TOF); % if bad fit or few points , quit
    return;
end

% check second wave (SV) doesn't cross main wave (SH)
SHSVlinesdiff=abs(out2.times(1:length(latpos))-out1.times);
[~,imin]=min(SHSVlinesdiff);
if and(imin~=1,imin~=length(SHSVlinesdiff))
    % lines are crossing, throw SV out
    out2=NanOut(latpos,TOFfit,TOF);
    return;
end

% if second wave (SV) still valid & behind/later than main wave (SH), reassign
if all(out2.times(1:length(latpos))>out1.times)
    outSVhold=out2;
    out2=out1;
    out1=outSVhold;
end
...
function out=NanOut(latpos,TOFfit,TOF)
out.speed=NaN;
out.R2=NaN;
out.latpos=latpos;
...
% go ahead and populate-- when using this script wont get to
out.TOF=TOF;
