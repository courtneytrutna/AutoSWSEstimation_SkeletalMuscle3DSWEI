function [out,out2] = Calc_RadonSum(plane,latmm,tms,radonsumparams,fignum,colorrange)
...
% radonsumparams is a structure with certain required field names, see
% below

minlatmm=radonsumparams.minlatmm;
maxlatmm=radonsumparams.maxlatmm;
minspeed=radonsumparams.minspeed;
maxspeed=radonsumparams.maxspeed;
tmsminstart=radonsumparams.tmsminstarttime;
tmsmaxstart=radonsumparams.tmsmaxstart;
tmsmaxend=radonsumparams.tmsmaxend;

origintmsthreshold=radonsumparams.origintmsthreshold;

promthres=.25; % this will be used to remove really small peaks in the multipeak fitting

% Define range over which to latSum
ilatstart = find(latmm>=minlatmm,1,'first');
ilatend   = find(latmm<=maxlatmm,1,'last');

% Adjust start time to earliest available, cut out pre-zero start points
% find index for maxendtimes
[~, minstarttimeindex]=min(abs(tms-tmsminstart));
[~, maxstarttimeindex]=min(abs(tms-tmsmaxstart));
[~, maxendtimeindex]=min(abs(tms-tmsmaxend));

[latsums]=FindLatSumsUsingEndpoints(plane,ilatstart,ilatend,minstarttimeindex,maxstarttimeindex,maxendtimeindex);

latsums = LimitMaxSpeed(latsums,latmm,tms,minspeed,maxspeed,ilatstart,ilatend);

latsums = LimitBackProjectionToOrigin(latsums,tms,latmm,ilatstart,ilatend,origintmsthreshold);

[itstart,itend,tms_start,tms_end,hittingedgeflag,ilocaltstart,ilocaltend,tms_start_local,tms_end_local,latsums_sub] = FindBestAndLocalPeaks(latsums,tms,minstarttimeindex,maxstarttimeindex,maxendtimeindex,promthres);
...
for istart=startTimeIndex:maxistart
    ...
    % above is equivalent to but faster than using sub2ind
...
function latsums = LimitMaxSpeed(latsums,lat,t,minspeed,maxspeed,ilatstart,ilatend)
...
function [itstart,itend,tms_start,tms_end,hittingedgeflag,ilocaltstart_sub,ilocaltend_sub,tms_start_local,tms_end_local,latsums_sub]=FindBestAndLocalPeaks(latsums,tms,minstarttimeindex,maxstarttimeindex,maxendtimeindex,promthres)
...
for itest=1:length(ilocaltstart)
    ...
    %remove sum-squared prominence less than promthres (good value is 0.25)
    if sqrt(p2(ilocaltstart(itest),ilocaltend(itest)).^2+p1(ilocaltstart(itest),ilocaltend(itest)).^2)<promthres
        idxremove(length(idxremove)+1)=itest;
    end
end
